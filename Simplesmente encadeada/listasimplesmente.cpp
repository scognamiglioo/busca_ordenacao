...
void gerarListaAleatoria(Lista *lista, int n){
    srand(time(NULL)); // 
    for(int i = 0; i < n; i++){
        int valor = rand() % 1000 + 1; // gera número aleatório entre 1 e 1000
        inserir(lista, valor); // insere o valor na lista
    }
}   
...
void swapPosicao(Lista *lista, int posicao1, int posicao2){
    ...
    //if(aux1 == NULL || aux2 == NULL){ //se aux1 ou aux2 for NULL
...
void buscaMenorValor(Lista *lista){
    ...
    while(aux != NULL){
        numAcessos++; // Increment for each node visited in the loop
